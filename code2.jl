...
#----------------------------------------------
H_max=2.5
I=100# previously "I" was my identity matrix, now I'm using it as a number
...
title!(L"$N=40+2,\Delta=0.2,t=1,t'=0.0$")
xlabel!(L"$\mu$")
...
for e_d in pruebas
	...
	for mu in vals
		...
		for tmr in espec
			if tmr>-0.1 && tmr <0.1
				if !legend_added
					display(scatter!([mu], [tmr], markercolor = esc, markerstrokecolor = esc, markersize = tama[letsgo],label=curvas2[letsgo],markeralpha = 0.5))
					legend_added=true
				else 
					display(scatter!([mu], [tmr], markercolor = esc, markerstrokecolor = esc, markersize = tama[letsgo],label="",markeralpha = 0.5))
				end
			end
		end	
