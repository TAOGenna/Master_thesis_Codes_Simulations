...
#------------------------------------------------------------------------------------------------------------#
#---------------PLOT SETTINGS--------------------------------------------------------------------------------#
min_energy=0.0001# aumentar esto para el caso de N=40
min_energy_=0.05
...
p=scatter(size = (width_pixels, width_pixels),
        xlims=(-H_max,H_max),
...
#=
separa=0.09
para_costado_derecho=0.3
para_abajo=0.25
x_position = -H_max + para_costado_derecho # Example x position
y_position = min_energy - separa - para_abajo # Example y position
annotate!(p, x_position, y_position - para_abajo, Plots.text(L"N=50", label_fontsize))
annotate!(p, x_position, y_position - separa - para_abajo, Plots.text(L"\Delta=0.2", label_fontsize))
annotate!(p, x_position, y_position - 2*separa - para_abajo, Plots.text(L"t=1", label_fontsize))
annotate!(p, x_position, y_position - 3*separa - para_abajo, Plots.text(L"t'=0.2", label_fontsize))
annotate!(p, x_position, y_position - 4*separa - para_abajo, Plots.text(L"V=1", label_fontsize))
=#
       ...
       if x==iterations
            ...
            for tmr in espec_plot
                if tmr>-min_energy && tmr<min_energy
                    ...
                    if !legend_added
                        display(scatter!([values_ed], [tmr], markercolor = "blue", markerstrokecolor = "blue", markersize = 1.2,label="",markeralpha = 0.8))
...
#filename_pdf = "Energy vs e_d, caso V=$(V) y N=$(N).pdf"
