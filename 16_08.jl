...
ALSO: PLOTS EXPECTATION VALUE VS e_d | Change the block that is in comments to turn on this option
...
for values_ed in through
    ...
    for x in 1:iterations
       ...
       if x==iterations
            ...
            # uncomment to plot expectation values #####################################
            #display(scatter!([values_ed], [initial_ev[3]], markercolor = "black", markerstrokecolor = "black", markersize = 2.2,label="",markeralpha = 1.2))
            ###########################################################################

            #comment this part to plot energy vs e_d ################################
            # espec_plot=eigvals(matrix_plot)
            # for tmr in espec_plot
            #     if tmr>-min_energy && tmr<min_energy
            #         #push!(information_to_csv,(values_ed,tmr))
            #         display(scatter!([values_ed], [tmr], markercolor = "blue", markerstrokecolor = "blue", markersize = point_size,label="",markeralpha = 0.8))
            #     end
            # end  
            #######################################################################           
       end
...
plot(store_x,store_y,
    ...
    legend=false, 
    #ylims=(-min_energy,min_energy), # min_energy
    ...
    linewidth=3,      # Linewidth for the curve
...
in_set_font = 25
separation = y_pos/17
# annotate!( x_pos, y_pos, Plots.text(L"N=50", in_set_font))
...
#plot(store_x, store_y, seriestype=:line)
#df = DataFrame(col1 = first.(information_to_csv), col2 = last.(information_to_csv))
#filename_csv = "t2 Expectation Value c+d c+1 V=$(V) y N=$(N).csv"
filename_pdf = "nd.pdf"
